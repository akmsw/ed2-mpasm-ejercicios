...
;   conectada al terminal RA4 y que saque ese valor en binario natural por
;   el puerto B. Sólo se utilizarán los bits RB0 a RB3 que son los que tienen
;   conectados diodos LED para su observación. Como consecuencia, el contador
;   es de 4 bits: de 0 a 15.
...
;-------------------INICIO DEL PROGRAMA-----------------------------------------
    ...
    LOOP    MOVFW   OUT		    ; Cargo OUT en W y lo muestro por PORTB.
	    ...
	    CALL    PRESSED	    ; Si está en '0', llamo a la subrutina.
	    GOTO    LOOP	    ; Sino, vuelvo a LOOP.
	    
 PRESSED    BTFSS   PORTA,4	    ; Hago polling en RA4 hasta que el pin
	    GOTO    $-1		    ; vuelva a estar en '1' y ahí incremento
	    CALL    SUM		    ; el valor a mostrar en PORTB.
	    RETURN
	    
    SUM	    INCF    OUT,1	    ; Incremento OUT y lo cargo en W.
	    MOVFW   OUT
